...
def train_model(
    ...
    if prev_losses is None:
        ...
        print("Applying Boosting with Ensemble of Models")
        print("Applying following losses to the model")
        print(prev_losses)
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(
            ...
            if not first_time:
                prev_loss = torch.tensor(prev_losses[count]).to(device)
                loss = loss * prev_loss
    ...
    with torch.no_grad():
        for batch in tqdm(
            train_loader, desc=f"Getting weights for next ensemble model"
        ):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            labels = batch["label"].to(device)
